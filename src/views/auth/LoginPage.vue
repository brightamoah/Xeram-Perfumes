<template>
  <div class="flex min-h-[60dvh] flex-col justify-center px-4 py-4 sm:px-6 sm:py-8 lg:px-8">
    <div class="mt-4 w-full sm:mx-auto sm:mt-8 sm:w-[90%] sm:max-w-md md:w-[80%] lg:w-[70%]">
      ...
      >
        <div class="mb-4 sm:mx-auto sm:mb-6 sm:w-full sm:max-w-md">
          <h2 class="text-center text-xl font-bold sm:text-2xl">Welcome Back To Xeram!</h2>
          <p class="text-center text-sm sm:text-base">Please login to continue</p>
        </div>

        <UForm :state="state" class="space-y-4 sm:space-y-6" @submit.prevent="handleLogin">
          <div>
          ...
          <div class="mt-6 mb-5 flex items-center justify-between">
            <div class="flex shrink-0 items-center">
              <UCheckbox
                ...
                label="Remember me"
                :ui="{
                  base: ' ring-(--ui-border-inverted) ',
                }"
              />
            </div>

            <div class="text-sm sm:text-sm">
              <a href="#" class="font-semibold whitespace-nowrap text-(--ui-text-highlighted)">
                Forgot password?
              </a>
            </div>
          </div>
          ...
          <div class="mt-4 flex items-center justify-center sm:mt-6">
            <UButton
              ...
              :loading="isLoading"
              class="flex w-full items-center justify-center rounded-full border border-(--ui-border) bg-black px-4 py-2 text-sm font-semibold text-white shadow-sm focus:ring-2 focus:outline-none sm:w-[85%] sm:py-2.5 sm:text-base md:w-[70%] dark:bg-white dark:text-black"
            />
          </div>
        </UForm>
        ...
        <p class="mt-4 text-center text-sm sm:mt-6 sm:text-sm">
          ...
          <router-link :to="{ name: 'signup' }" class="ml-1 border-b-2 text-base font-semibold">
            Sign Up
          </router-link>
        </p>
      </div>
